...
void render() {
	...
	if (menu_open)
	{
		...
		if (ImGui::Begin("cheat", 0,  ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoScrollbar |  ImGuiWindowFlags_NoSavedSettings))
		{
			TssakliMenu();
		}ImGui::End();
	}
...
void stop_render() {
	...
	DestroyWindow(Window);
	UnregisterClassW((_(L"test")), NULL);
}
...
void create_window() {
	...
	Window = CreateWindowExW(NULL, _(L"test"), _(L"test"), 0x80000000L | 0x10000000L, 2, 2, Width - 2, Height - 2, 0, 0, 0, 0);
